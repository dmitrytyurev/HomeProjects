...
const int UINT_NO_VALUE = 255;

//-------------------------------------------------------------------------------


namespace AttributePropertyType // Тип атрибута таблицы. Часть типов используется для сериализации/десериализации, часть типов для отображения данных в таблице
{
	enum : uint8_t
	{
		Translation_t = 0,  // Текст одного из дополнительных языков
		CommonText_t = 1,   // Текст общего назначения (не перевод)
		UintValue_t = 2,    // Значение целого типа
		...
		TranslationStatus_t = 8,   // Статус актуальности перевода (соответствует ли он базовому тексту)
	};
}
...
namespace EventType
{
	enum : uint8_t
	{
		...
		ChangeAttributeInText = 112,   // Изменилось значение атрибута в тексте (если указанного атрибута нет в тексте, то он создаётся; если назначается пустой текст, то атрибут удаляется из текста. Если тип - целое число и присылается UINT_NO_VALUE, то удаляется (нет значения))
	};
}
