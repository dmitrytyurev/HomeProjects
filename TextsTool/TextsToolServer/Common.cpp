...
void SMessagesRepaker::Update(double dt)
{
	...
	for (auto& client : _app->_clients) {
		...
		if (client->_sessionId == clLow->_sessionId) { // Если они не совпадут, значит создался новый SConnectedClientLow, а SConnectedClient ещё не успел пересоздаться, он сделает это на следующем Update и тогда мы снова придём сюда и скопируем пакеты уже в нового SConnectedClient
